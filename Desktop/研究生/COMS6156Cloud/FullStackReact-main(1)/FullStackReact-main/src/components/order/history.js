...
const History = () => {
    ...
    return (
        <div>
            ...
            <button className='button' onClick={prevPageClickHandler} disabled={page<=1}>{"<<"}</button>
                {page}
            <button className='button' onClick={nextPageClickHandler} disabled={page>=maxPage}>{">>"}</button>
        </div>
    );
};
